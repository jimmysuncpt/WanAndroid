        ...
        try {
            ...
            if (buildProperties.containsKey(KEY_MIUI_VERSION_NANE) || buildProperties.containsKey(
                ...
                if (buildProperties.containsKey(KEY_MIUI_VERSION_NANE)) {
                    ...
                    if (!TextUtils.isEmpty(versionName) && versionName.matches(Regex("[Vv]\\d+"))) { // V8
                        try {
                            rom.baseVersion =
                                versionName.split("[Vv]").toTypedArray()[1].toInt()
                        } catch (e: Exception) {
                            e.printStackTrace()
                        }
                    }
                ...
                if (buildProperties.containsKey(KEY_MIUI_VERSION)) {
                    ...
                    if (!TextUtils.isEmpty(versionStr) && versionStr.matches(Regex("[\\d.]+"))) {
                ...
                if (buildProperties.containsKey(KEY_COLOROS_ROM_VERSION)) {
                    ...
                    val matcher =
                        Pattern.compile("ColorOS([\\d.]+)").matcher(versionStr) //
                    // ColorOS2.1
                    if (!TextUtils.isEmpty(versionStr) && matcher.find()) {
                        try {
                            val version = matcher.group(1)
                ...
                if (buildProperties.containsKey(KEY_FUNTOUCHOS_OS_VERSION)) {
                    ...
                    if (!TextUtils.isEmpty(versionStr) && versionStr.matches(Regex("[\\d.]+"))) { // 3.0
                        try {
                            rom.version = versionStr
                            rom.baseVersion = versionStr.split("\\.").toTypedArray()[0].toInt()
                        } catch (e: Exception) {
                            e.printStackTrace()
                        }
                    }
                ...
                if (buildProperties.containsKey(KEY_EUI_VERSION)) {
                    ...
                    val matcher =
                        Pattern.compile("([\\d.]+)[^\\d]*").matcher(versionStr) //
                    // 5.9.023S
                    if (!TextUtils.isEmpty(versionStr) && matcher.find()) {
                        try {
                            val version = matcher.group(1)
                ...
                if (buildProperties.containsKey(KEY_DISPLAY_ID)) {
                    val versionStr =
                        buildProperties.getProperty(KEY_DISPLAY_ID)
                    val matcher =
                        Pattern.compile("amigo([\\d.]+)[a-zA-Z]*")
                            .matcher(versionStr) //
                    // "amigo3.5.1"
                    if (!TextUtils.isEmpty(versionStr) && matcher.find()) {
                        try {
